...
namespace qreg
{
  ...
  IfOpenUff::IfOpenUff(std::string newBinaryFilePath  ,
  ...
  IfOpenUff::IfOpenUff(IfOpenUff&& ifOpenGff)
  {
  ...
  auto
  IfOpenUff::operator=(IfOpenUff&& ifOpenGff)
  ->IfOpenUff&
  {
  ...
  auto
  IfOpenUff::getNormalTerminationMessage(void)const noexcept
  ->std::string
  {
  ...
  ->void
  {
    ...
    outputFilePath_.append("/uff.grad");

    std::string inputFilePath = outputFolderPath_;
    inputFilePath.append("/uff.xyz");
    ...
    if(std::system(purge.c_str()) != 0)
    {
      ...
      std::cerr << "Unable to purge old OpenUFF files, aborting Qreg."
                << std::endl                                      ;
